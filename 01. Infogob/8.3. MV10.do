...
use `middle_infogob'\hombremujer10_append, clear
gen id=_n
...
keep if _merge==3
mmerge iddistrito using "`middle_infogob'\hombremujer10_append"
keep if _merge==3
rename id distrito_ubigeo

gen anho_cargo=1 if anho==2006
replace anho_cargo=2 if anho==2010
replace anho_cargo=3 if anho==2014
replace anho_cargo=3 if anho==2018
replace anho_cargo=3 if anho==2022

save `middle_infogob'\hombremujer10_append_ubigeo, replace
